...
L=2; %full width at half height, arcmin
%L=4;
% x=[-1:.1:1]*45;
psf_size=80; % INCREASE TO MAKE RING MORE APPARENT
Xp=1:psf_size;
XP=((Xp / psf_size)) - .5;
...
stimulus_neur = solar_limb_nonlinear + (venus_nonlinear - 1);
...
%------------------------Figures-----------------------------------
figure(1);
subplot(2,2,1);imagesc(stimulus_figure, [0 1] ); colormap(gray); axis image; xlim([760,840]);ylim([220,300]);
%hold on; plot([imSize/2 imSize/2],[1 imSize], 'r'); hold off;
subplot(2,2,2); plot(stimulus_figure(:,imSize/2),'r');axis square;
hold on; plot(solar_limb_nonlinear(:,imSize/2),'b'); hold off;
subplot(2,2,3);plot(venus_nonlinear(200:350,imSize/2));axis square;
