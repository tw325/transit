...
sigma = 60;    % gaussian standard deviation in pixels
...
% L50W = 0.1;
gaussW=(gaussW*amp);
%gaussW(gaussW>1)=1;% INSIGNIFICANT
curve = diag(gaussW);
subplot(2,4,3);plot(curve); ylim([-2 30]); axis square; ylim([-2, 16]);   
hold on;
plot([0 400],[1 1],'r');
hold off;
...
subplot(2,4,4); plot(nonlinear);colormap(gray); axis square;
gaussWNL(gaussWNL>1)=1;

curve = diag(gaussWNL);
subplot(2,4,5);plot(curve); ylim([-2 30]); axis square; ylim([-2, 16]);   
subplot(2,4,6); plot(diag(gaussWNL)); colormap(gray); axis square; ylim([-1.5 1.5]);
gaussWNL = gaussWNL*2-1;
subplot(2,4,7); plot(diag(gaussWNL)); colormap(gray); axis square; ylim([-1.5 2.5]);
temp = [1 100];
