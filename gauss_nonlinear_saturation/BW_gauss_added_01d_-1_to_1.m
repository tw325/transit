...
trim = .05;    % trim off gaussian values smaller than this
x_pos=round(imSize*.285); % centr pos =24,24
y_pos=round(imSize*.285);
%%
%=======make linear ramp
X = 1:imSize;             % X is a vector from 1 to imageSize
...
limy = [1,imSize/4];
figure(4);clf;subplot(2,3,1);imagesc( gaussWB, [-1 1] ); colormap(gray); axis on; axis image; title('Linear');
subplot(2,3,2);imagesc( gaussWNLB, [-1 1] ); colormap(gray); axis on; axis image; title('White Nonlinear'); 
subplot(2,3,3);imagesc( gaussWNLBNL, [-1 1] ); colormap(gray); axis on; axis image; title('White Nonlinear Black Nonlinear'); %xlim(limx);ylim(limy);     
% use gray colormap
...
subplot(2,3,5);plot(diag(gaussBNL));ylim([-amp amp]);
subplot(2,3,6);plot(curve3);hold on; plot(diag(gaussWNL),'k');hold off;ylim([-amp amp]);
