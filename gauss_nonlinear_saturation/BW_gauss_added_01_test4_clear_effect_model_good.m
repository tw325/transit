...
% gaussW=gaussW*3;
subplot(2,4,7);plot(curve); ylim([-2 30]); axis square; ylim([-2,2]);
hold on
plot(diag(gaussBNL),'r');ylim([-2 30]); axis square; ylim([-2,2]);
hold off;
...
gaussWNLBNL(gaussWNLBNL<-1)=-1;
subplot(2,4,8);plot(diag(gaussWNL),'r'); colormap(gray); axis square; ylim([-1.5,1.5]);
hold on; plot(diag(gaussWNLBNL)); colormap(gray); axis square; ylim([-1.5,1.5]);
hold off;
